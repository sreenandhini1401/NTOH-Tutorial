program poisson_fft
        ...
        integer:: i,j,k,n1
        real:: length, hht,dl,dh,nl,nh,a,b,nnt,ntotal
        real:: pi,tm1,tm2,tt, const,x,x_,y
        real, allocatable:: f(:,:)
        ...
        open(30, file= 'o.txt',status='unknown')
        do i= 1, n
            write(30,*), f(i,:)
        end do
        close(30)
		...
		call sinft2d(f,n,n,n,1)
        print*, f(2,4)

        do  i=2,n
            do  j=2,n
                const=tt*(+cos(tm1*(i))+cos(tm2*(j))-2.0)
                f(i,j)=f(i,j)/const
            end do
        end do
        print*, f(2,4)
